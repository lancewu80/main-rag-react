...
function QAPanel({ type }) {
  ...
  const config = {
    ...
    web: {
      title: '🌐 網路資訊分析',
      description: 'AI 分析網路相關資訊',
      icon: <Web sx={{ fontSize: 40 }} />,
      color: 'info',
    },
    hybrid: {
      title: '🔀 綜合 AI 分析',
      description: '綜合多方面知識的 AI 回答',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: messages[answerQuality] || '回答生成完成',
        severity: 'success'
      });
            ...
            <Box sx={{ mt: 3, p: 2, bgcolor: 'info.50', borderRadius: 2 }}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} md={3}>
                  ...
                  <Typography variant="body2" fontWeight="bold">
                    {type === 'rag' ? '智能問答' : type === 'web' ? '網路分析' : '綜合分析'}
                  </Typography>
                </Grid>
                <Grid item xs={6} md={3}>
                  <Typography variant="caption" color="text.secondary">AI 狀態</Typography>
                  <Typography variant="body2" fontWeight="bold" color={answer.metadata.ai_available ? 'success.main' : 'error.main'}>
                    {answer.metadata.ai_available ? '已連接' : '未連接'}
                  </Typography>
                </Grid>
              </Grid>
            </Box>
            ...
            <CardContent>
              ...
              <Grid container spacing={2}>
                {[
                  ...
                  { name: '網路資訊分析', status: 'active', icon: <Web /> },
                  { name: '綜合 AI 分析', status: 'active', icon: <Merge /> },
            ...
            <Box sx={{ mt: 4 }}>
              ...
              <Paper sx={{ p: 3 }}>
                ...
                <Alert severity="info" sx={{ mt: 2 }}>
                  💡 提示：確保 Ollama 服務正在運行以獲得最佳 AI 回答體驗
                </Alert>
              </Paper>
            </Box>
          </Box>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={9}>
            ...
            <Box sx={{ mt: 4, textAlign: 'center', color: 'text.secondary' }}>
              <Typography variant="body2">
                © 2024 AI 智能問答系統 | FastAPI + React + Ollama
              </Typography>
              <Typography variant="caption" display="block">
                版本 1.0.0 | Ollama 驅動 AI 版本
              </Typography>
            </Box>
          </Grid>
        </Grid>
